package app.pivo.android.mlkit_camerax.common

import android.content.Context
import android.graphics.Canvas
import android.util.AttributeSet
import android.view.View
import app.pivo.android.mlkit_camerax.common.GraphicOverlay.Graphic
...
class GraphicOverlay(
    ...
    abstract class Graphic(private val overlay: GraphicOverlay) {
         ...
         * Adjusts the x coordinate from the preview's coordinate system to the view coordinate system.
         */
        open fun translateX2(y: Float): Float {
            return if (overlay.facing == CameraSource.CAMERA_FACING_FRONT) {
                overlay.width - scaleX(y)
            } else {
                overlay.width - scaleX(y)
            }
        }

        open fun translateX3(y: Float): Float {
            return if (overlay.facing == CameraSource.CAMERA_FACING_FRONT) {
                scaleX(y)
            } else {
                scaleX(y)
         ...
         * Adjusts the y coordinate from the preview's coordinate system to the view coordinate system.
         */
        open fun translateY2(x: Float): Float {
            return if (overlay.facing == CameraSource.CAMERA_FACING_FRONT) {
                overlay.height - scaleY(x)
            } else {
                scaleY(x)
            }
        }

        open fun translateY3(x: Float): Float {
            return if (overlay.facing == CameraSource.CAMERA_FACING_FRONT) {
                scaleY(x)
            } else {
                overlay.height - scaleY(x)
